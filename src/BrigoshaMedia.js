...
const SOCKET_SERVER = 'wss://2j5zd8k29h.execute-api.ap-south-1.amazonaws.com/development'
const HEART_BEAT_URL = 'http://65.1.166.36:3000'
...
export default class BrigoshaMedia {
  ...
  constructor() {
    ...
    this.socket = null;
    this.baseUrl = 'http://13.233.216.74:1000/api';
...
//                     platfor:'android',confid=int(),mode:'online'
// //user{
//   name:'ujjwal'
//   id:Int
// }
 async init(user_id, room_id, userName, user, isTeacher, platform, confId, mode) {
   
    this.user_id = parseInt(Math.random()*100, 10);
    ...
    console.log('generated userid = =' , this.user_id)
        ...
        case 'userJoined':
          {
            ...
            console.log('UserJoined',payload.data.user)
          }
          break;
        case 'userRejoined':
          {
        ...
        case 'userLeft':
          {
            ...
            console.log('UserLeft',payload.data.user)
  ...
  async getRouterRtpCapabilitiesRcv(data) {
    try {
      console.log('detected device ==',mediasoupClient.detectDevice())
  ...
  async loadDevice(routerRtpCapabilities) {
    ...
    try {
      // device = new mediasoupClient.Device({ handlerName: 'ReactNative' });                   //ujjwal modified
      device = new mediasoupClient.Device({ handlerName:mediasoupClient.detectDevice()  });
    } catch (error) {
  ...
  createWebRtcTransportProducerRcv(data, produceCallBack) {
    ...
    try {
      ...
      console.log('produce trans errr',e);
    }
    ...
    Emitter.emit('producersCreated',null)
  }
  ...
  async getProducersRcv(body) {
    // console.log('get producer event fired ----------->>>',body)
    //EventRegister.emit('mediaReady', body);
    await this.newProducers(body.producers);
   await  this.newDataConsumers(body.consumersToSubscribe);
   Emitter.emit('allConsumeDone',null)
  }
  ...
  async produce(type) {
    ...
    try {
      ...
      if (!audio) {
        ...
        // console.log('Local media ->>>>>',this.localMedia)
      ...
      producer.on('transportclose', () => {
        console.log('producer trasport close')
        if (!audio) {
          this.localMedia.stream.getTracks().forEach(track => {
            track.stop();
          });
  ...
  async consume(producer_id, userId) {
    await this.getConsumeStream(producer_id, userId);
  }
  ...
  async getConsumeStream(producerId, userId) {
   ...
   await this.getConsumeStreamRcv(consumeResponse);
  }
  ...
  pauseProducerRemote(producer_id) {
    this.producers.get(producer_id).pause();
    Emitter.emit('remotevideopaused',null)
  }
